...
// =========================
// MAIL SETTINGS SAGA HANDLERS
// =========================

function* fetchMailSettings(action) {
  try {
    const response = yield call(getMailSettingsUrl);
    yield put(onGetMailSettingsSuccess(response.data));
    // toast.success(response.message);
  } catch (error) {
    toast.error(error.message);
    toast.error(error.response?.data?.message);
    yield put({ type: UPDATE_MAIL_SETTINGS_FAIL });
  }
}

function* updateMailSettings(action) {
  try {
    const response = yield call(updateMailSettingsUrl, action.payload);
    yield put({ type: GET_MAIL_SETTINGS });
    // toast.success(response.message);
  } catch (error) {
    toast.error(error.message);
    toast.error(error.response?.data?.message);
...
function* mailsSaga() {
  ...
  yield takeEvery(GET_MAIL_SETTINGS, fetchMailSettings);
  yield takeEvery(UPDATE_MAIL_SETTINGS, updateMailSettings);
}
